...
import {styles} from '../../styles/interfazCuidador/EditMedicineScreen.styles';
...
const normalizeCategory = (category) => {
  if (category === 'Tableta') return 'Tableta / Cápsula';
  if (category === 'Jarabe') return 'Jarabe / Gotas';
  if (category === 'Otro') return 'Insumo médico';

  return category || 'Tableta / Cápsula';
};
...
const EditMedicineScreen = ({ settings, medicine, onCancel, onSaved, patientId }) => {
  ...
  const initialCategory = normalizeCategory(medicine?.category);
  ...
  const [doseAmount, setDoseAmount] = useState(String(medicine?.doseAmount ?? 1));
  const [dailyDose, setDailyDose] = useState(String(medicine?.dailyDose ?? ''));
  const [category, setCategory] = useState(initialCategory);
  const [saving, setSaving] = useState(false);

  const [reminderEnabled, setReminderEnabled] = useState(
    medicine?.reminderEnabled !== false
  );
  const [schedules, setSchedules] = useState(
    Array.isArray(medicine?.schedules) ? medicine.schedules : []
  );
  ...
  const handleSave = async () => {
    ...
    if (!currentStock.trim() || isNaN(Number(currentStock))) {
      Alert.alert('Dato inválido', `Ingresa una cantidad actual válida en ${stockUnit}.`);
      return;
    }
    ...
    if (Number(currentStock) < 0) {
      Alert.alert('Dato inválido', 'La cantidad actual no puede ser negativa.');
      return;
    }
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    {
                      backgroundColor: selected ? colors.primary : inputBackground,
                    },
                  ]}
